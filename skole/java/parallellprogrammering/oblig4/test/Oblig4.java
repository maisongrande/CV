class Oblig4 {
    ...
    double[] seqTime, parTime, fulTime;
    ...
    public Oblig4(String[] args) {
        ...
        System.out.println("Test av Konveks Innhylling"
                ...
                +(nIter>1?" ganger.":" gang."));
        System.out.printf("%8s%20s%20s%18s%12s%15s%15s\n", "Punkter:", "Sekvensiell Tid:",
                "Parallell tid:", "'Full'-Para tid:",
                "Sek/Par:", "Sek/Full:", "Para/Full:");
        execute();
    }
    ...
    void execute() {
        ...
        fulTime = new double[nIter];
        int antTraader = Runtime.getRuntime().availableProcessors();

        ParallellKohyll pk = new ParallellKohyll();
        FullParallellKohyll fpk = new FullParallellKohyll();

        for (int N = upperN; N >= lowerN; N/=divisor)
        {
            x = new int[N];
            y = new int[N];
            n = N;
                    ...
                    run++)
            {
                ...
                tic = System.nanoTime();
                IntList fullParaKH = fpk.paraKohyllInit(punkter);
                fulTime[run] = ((System.nanoTime() - tic)/1000000.0f);
                if (TEGNUT) { tu = new TegnUt(this, fullParaKH, "'FULL'-PARALLELL"); }
            }
            showResult(N);
        }
        fpk.pir.exit();
    ...
    void showResult(int run) {
        ...
        System.out.printf("%-12d %-20.5f %-15.5f %-20.5f %-14.3f %-15.3f%-16.3f\n",
                run,
                arrayMedian(seqTime),
                arrayMedian(parTime),
                arrayMedian(fulTime),
                (arrayMedian(seqTime) / arrayMedian(parTime)),
                (arrayMedian(seqTime) / arrayMedian(fulTime)),
                (arrayMedian(parTime) / arrayMedian(fulTime)));
...
class Punkter {
    ...
    /** Kalles baade av sekvensiell og parallell version */
    static void
