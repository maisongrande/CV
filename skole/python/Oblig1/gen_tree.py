...
parser.add_argument('--rec_depth', type = int,  nargs=1, default=1,
...
def generate_tree(target, numdirs,  numfiles, depth, verbose=False):
    """
...
numfiles: int
    ...
    sub_d = [random_string(rdint(2,22)) for i in range(depth)] 
    ...
    for elem in sub_d: 
        ...
        for i in range(numdirs):
            sub = random_string(rdint(2,22))     # get rand number of acceptable file size length
            sub = os.path.join(os.getcwd(), sub) # combine this name to current directory we are in
