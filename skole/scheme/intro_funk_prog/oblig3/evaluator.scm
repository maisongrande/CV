...
(define (mc-eval exp env) ;; tilsvarer eval i SICP
  ;; Eval: leser inn et uttrykk og et mijo. Det er Eval sin oppgave aa
  ;; lete opp variblene i uttrykket gitt.
  ;;
  (cond ((self-evaluating? exp) exp)
	;; Sjekk om uttrykket er selv evaluerende, altsa hvis man skriver
	;; det inn pa repl-et kommer samme i retur, her -> exp returneres
	;; prosedyren returnerer true for: string, boolean og nummer/
	((variable? exp) (lookup-variable-value exp env))
	;; En variabel er en navngitt binding til en verdi og denne 
	;; variablen paklistres ett ' naar det lages.
	;; Hvis det er en variabel letes det rekursivt etter navnet
	;; til variablen i miljoet og de omkringliggende miljoene.
	;; Verdien til denne variablen returneres.
	((special-form? exp) (eval-special-form exp env))
	;; Sjekker om det er et av foelgende:
	;; (quote), tilordning, define, if, begin eller cond.
	;; Er det en lambda vil make-procedure kalles. Se make-proc
        ((application? exp)
	 ;;
	 ;;
...
(define (lookup-variable-value var env)
  ;; Ved treff pa cond variable:
  (define (env-loop env)
    (define (scan vars vals) 
      ;; parallell rekursjon på listene av symboler og verdier
      ;; Sjekker om variablen som det letes etter finnes i variabel/verdi
      ;; parene som dette miljoet inneholder. Hvis ikke funnet i dette
      ;; miljoet, sa sjekkes neste eventuelt frem til funn/ikke-funn.
      ;; Hvis det blir funnet returneres kun verdien (cdr av paret)
      ;; Hvis det ikke blir funnet vil neste omkringliggende miljo sjekkes.
      (cond ((null? vars)
             (env-loop (enclosing-environment env)))
            ((eq? var (car vars))
             (car vals))
            (else (scan (cdr vars) (cdr vals))))) ;; Slutt scan
    
    (if (eq? env the-empty-environment)
        (error "Unbound variable:" var)
        (let ((frame (first-frame env))) ;; car av environment listen
          (scan (frame-variables frame)  ;; car av denne listen
                (frame-values frame))))) ;; cdr av dette paret  
  (env-loop env))
